...
Lock stopsLock = new Lock();
Lock menuLock = new Lock();
...
Future<List<FoodTruckStop>> retrieveStops() async {
    List<FoodTruckStop> stops = await stopsLock.synchronized(() async {
...
Future<List<FoodTruckMenu>> retrieveMenus() {
    return menuLock.synchronized(() async {
        List<dynamic> jsonList = await jsonCache.retrieveJson(key: Identifier.foodTruckMenu, urlIfNeeded: PageRouteConfig.getFoodTruck('menu'));

        if (jsonList == null || jsonList.isEmpty) {
            return [];
        }

        return jsonList.map((j) => FoodTruckMenu.fromJson(j as Map<String, dynamic>)).toList();
    });
}